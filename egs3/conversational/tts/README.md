...
**`TurnTakingJudgeMetric`** (Talking Turns judge, arXiv 2503.01174 / ESPnet `egs2/swbd/slu1`; checkpoint `espnet/Turn_taking_prediction_SWBD`, CC-BY-4.0; design note "Design - Talking Turns Judge Turn-Taking Evaluation" in the vault):

The statistics above say how often events happen and how long they last, never whether an event happened at a plausible MOMENT.
The judge is a causal classifier (frozen Whisper-medium encoder + linear head) that predicts every 40 ms, from up to 30 s of the mono mixdown, which event comes next: `C` continuation, `NA` silence, `IN` interruption, `BC` backchannel, `T` turn change.
Realized events come from the same per-channel Silero IPUs as `InteractionMetric`, rasterised onto the judge's grid and run through the upstream Switchboard label state machine (ported verbatim, state reset per window).
Generated audio has no backchannel annotation, so a proxy relabels an IPU as `BC` when it is <= 1.08 s (p95 of the 60,917 annotated Switchboard backchannel durations), starts while the other channel holds the floor, and the floor does not pass to it afterwards; the cap is the `bc_max_sec` knob and is applied identically to every system, gt anchor included.
Two-channel windows only (one-channel windows get a silent partner; 3+ channels are skipped and counted in `judge_skipped_windows`).
Mono-ingested baselines (`ingest.output: mixture`) have no per-channel activity and are NOT scorable by this labeler; a diarization-based labeler is a separate work item, and Silero-labelled judge scores belong to the channel-output table only.

- `judge_f1_{C,NA,IN,BC,T}`, `judge_f1_macro` - per-class macro-F1 of the judge's hard label (upstream `LabelThreshold` cascade) vs the realized label, pooled over every chunk of the run; `null` for a class absent from the realized labels; `judge_f1_macro` averages the defined classes.
- `judge_auc_{C,NA,IN,BC,T}`, `judge_auc_mean` - one-vs-rest ROC-AUC of the judge's soft likelihood vs the realized label, same pooling (this is exactly how the paper validates the judge on human-human Switchboard test, 92.0 overall).
- `judge_acc_turn_change` / `judge_acc_pause` - after the floor-holder paused (>= 3 silent chunks), accuracy of the other speaker's decision to take the turn (`T`) / let them continue (`C`) against the judge's `T - C` margin; `judge_acc_interrupt` / `judge_acc_no_interrupt` - the same for starting an overlap while the floor-holder is mid-speech; `judge_acc_bc` / `judge_acc_no_bc` - backchanneling or not; `judge_acc_willing_pause` / `judge_acc_willing_turn` - the floor-holder's side of the pause question; `judge_acc_interrupt_unsuccess` / `judge_acc_interrupt_success` - whether an interruption won the floor. These are the paper's role-conditioned metrics; our dialogues have no AI/human role, so each is computed under BOTH speaker-role assignments and the decision arrays are pooled before the accuracy is taken (`report_role_metrics: true`). `null` when the selection is empty. The upstream `only_AI` / `only_human` filters are NOT used: they filter the decision and judge arrays but leave `turn_arr` unfiltered, which misaligns the floor state (measured: 5-13 point shifts with array order); the five metric methods condition on the floor state themselves, so the aligned unfiltered arrays are the correct input.
- `judge_matched_chunks` / `judge_expected_chunks` - chunks joined between likelihoods and labels vs label rows; the metric RAISES if fewer than 99% match (grid drift), because the upstream join skips mismatches silently. `judge_bc_proxy_count` - IPUs relabelled by the proxy.
