...
def select_records(
    ...
    """Pick the evaluated subset: length band first, then optional subsample.

    Returns ``(indices, counts)``.  ``counts`` reports the two exclusion
    reasons SEPARATELY (``n_out_of_band`` / ``n_not_sampled``) because they
    mean opposite things: an out-of-band dialogue could not be generated in
    this model's trained regime, while a not-sampled one simply was not
    asked for.  Collapsing them into one "skipped" number would read as a
    mass failure whenever a small ``num_dialogues`` is set.
