...
def tiny(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        DualPromptInference,
        "_vocode",
        lambda self, mel: torch.zeros(mel.shape[-1] * HOP),
    )
    ...
    return DualPromptInference(
        str(tc), str(ck), str(tokens), str(stats), device="cpu", lowpass_hz=None
    )


def test_build_inputs_matches_dataset_layout(tiny):
    spk = (np.random.randn(24000 * 2 + 100) * 0.05).astype(
        np.float32
    )  # not hop aligned
...
def test_inference_configs_load():
    ...
    assert (
        "lang_prompt_speech" in a.input_key and "lang_prompt_speech" not in b.input_key
    )
