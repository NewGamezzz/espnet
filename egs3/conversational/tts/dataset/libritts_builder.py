        ...
        for split, subsets, hours_cap in splits:
            ...
            for entry, duration in pairs:
                ...
                if not text:
                    ...
                    continue
                records.append(
                    utterance_record(entry, duration, expected_rate, text)
                )
            n = write_window_manifest(
            ...
            print(
                f"    duration[s]: {_distribution([r.duration for r in records])}"
            )
...
def main() -> None:
    ...
    builder.prepare_source(
        recipe_dir=args.recipe_dir, dataset_root=args.dataset_root
    )
    if builder.is_built(recipe_dir=args.recipe_dir) and not args.force:
        print(
            f"Already built under {args.recipe_dir}/data; use --force to rebuild."
        )
        return
    builder.build(
