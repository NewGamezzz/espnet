...
def _meta(tmp_path):
    ...
    for lang, vids in (("de", ["v1", "v2", "v3"]), ("zh", ["1", "2", "3"])):
        for v in vids:
            key = f"de_{v}AAAAAAAAA-00001-00000000-00000500" if lang == "de" else f"zh_emilia_zh_000000000{v}"
            rel = f"{lang}/{v}.flac"
            p = root / rel
            p.parent.mkdir(parents=True, exist_ok=True)
            sf.write(p, np.zeros(16000 * 5, np.float32), 16000, format="FLAC", subtype="PCM_16")
            words = [{"word": f"w{i}", "start": i * 1.0 + 0.1, "end": i * 1.0 + 0.9, "score": 1.0}
                     for i in range(5)]
            rows.append({"key": key, "file_name": rel, "dur": 5.0,
                         "txt": " ".join(w["word"] for w in words), "align": {"words": words}})
...
def test_manifest_and_clips(tmp_path):
    ...
    # boundary nearest 30% of 5 s = end of w1 (1.9 s); target = start of w2 (2.1 s) to end
    assert text == "w2 w3 w4"
    assert abs(sf.info(spk).duration - 1.9) < 1e-6 and abs(sf.info(gt).duration - 2.9) < 1e-6
...
def test_eval_dataset_keys(tmp_path):
    ...
    assert set(s) >= {"utt_id", "text", "lang", "raw_text", "spk_prompt_speech", "lang_prompt_speech",
                      "ref_wav_path", "lang_ref_wav_path", "gt_wav_path"}
