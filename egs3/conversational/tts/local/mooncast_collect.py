r"""Verify a MoonCast run and gather its wavs for ingest.

``mooncast_infer.py`` already writes ``<window_id>.wav``, so like the
FireRedTTS-2 collector this script renames nothing - the id map was never a
...
* EVERY expected dialogue must be present, exactly once, with audio on disk.
  ...
  three dialogues exits cleanly and looks fine.
* A runaway turn is FLAGGED.  MoonCast's own cap is ``max_new_tokens =
  200 * 50``, i.e. 200 seconds on EVERY turn, which no turn of this test set
  could reach - so unlike FireRedTTS-2, where the cap itself was the thing
  to watch, a long turn here means the model never emitted its end token.
  The threshold is therefore ours and is set from what a turn of this
  material plausibly is, not from their ceiling.  This is why the runner
  records per-turn frame counts.
...
#: Their semantic token rate (``frame_size = 480`` samples at 24 kHz).
FRAMES_PER_SEC = 50
#: OUR runaway threshold, in frames: 30 seconds of continuous speech from
#: one turn.  The longest reference turn on this set is far shorter, and
#: their own cap sits at 200 s, so anything past this is an EOS failure
#: rather than a clipped generation.
RUNAWAY_FRAMES = 30 * FRAMES_PER_SEC
...
def verify(
    ...
    runaway_frames: int = RUNAWAY_FRAMES,
) -> dict:
...
def main() -> None:
    ...
    if report["runaway"]:
        ...
        print(
            f"RUNAWAY TURNS (>= {RUNAWAY_FRAMES} frames = "
            f"{RUNAWAY_FRAMES // FRAMES_PER_SEC} s of one turn): {rows}"
        )
