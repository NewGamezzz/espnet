...
class ConversationalTextPreprocessor(AbsPreprocessor):
    ...
    Mode T samples (``record.timestamp_text`` set) carry ``timestamp_text``
    (``True``), ``target_t0`` (float, seconds) and ``target_frames`` (int)
    keys instead of the ordinary flat branch text: the target region gets
    ...
    def __call__(self, uid: str, data: dict[str, Any]) -> dict[str, Any]:
        if data.get("timestamp_text"):
            ...
            # dataset guarantees target_t0/target_frames are present and the
            # planner's fit gate guarantees assembly succeeds - a raise below
            # is a pipeline bug.
